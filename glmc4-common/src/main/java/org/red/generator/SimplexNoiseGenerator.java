...
public class SimplexNoiseGenerator implements IGenerator {
    ...
    public float generate(float... coords) throws Exception {
        if (coords.length == 2)
            return 8 + 4 * SimplexNoise.noise((coords[0]) * _scale, (coords[1]) * _scale) + (SimplexNoise.noise((coords[0]) * _scale, (coords[1]) * _scale) * 0.5f) + (SimplexNoise.noise((coords[0]) * _scale, (coords[1]) * _scale) * 0.25f);
